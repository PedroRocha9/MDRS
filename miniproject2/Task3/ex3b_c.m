...
for pair = 1:size(anycastNodesList, 1)
    ...
    if bestEnergy < globalEnergy
        globalSol = bestSol;
        globalLoads = bestLoads;
        globalMaxLoad = bestLoad; 
        globalEnergy = bestEnergy;
        globalTime = bestLoadTime;
        bestNodes = anycastNodes;
    end
end

changedLinks = '';
for i = 1 : size(globalLoads, 1)
    if max(globalLoads(i, 3:4)) > 50
        changedLinks = append(changedLinks, ' {', num2str(globalLoads(i,1)), ', ', num2str(globalLoads(i,2)), '}');
    end
end

fprintf("E = %.2f \t W = %.2f \t No. sols = %d \t time = %.2f\n", globalEnergy, gloablMaxLoad, contador, globalTime);
fprintf("List of links that changed to 100Gbps:%s\n", changedLinks);